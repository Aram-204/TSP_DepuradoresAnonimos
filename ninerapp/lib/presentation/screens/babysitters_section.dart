...
class BabysittersSection extends StatefulWidget {
  const BabysittersSection({super.key});
...
class _BabysittersSectionState extends State<BabysittersSection> {
  ...
  bool hasPhisicalDisabilityExp = false;
  ...
  Future<void> _loadBabysitters() async {
    ...
    try {
      final babysittersRes = await _babysitterRepository.getBabysitters(minimumStars, minDistanceMts, maxDistanceMts, minExpYears, maxExpYears, minPricePerHour, maxPricePerHour, hasPhisicalDisabilityExp, hasVisualDisabilityExp, hasHearingDisabilityExp);
      setState(() {
